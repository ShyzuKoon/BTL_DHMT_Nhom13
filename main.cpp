...
//doi tuong tinh tien
GLfloat chair_table_x = 0.0;
GLfloat chair_table_z = 0.0;
...
void windown(float tmp) {
	...
	//khung phai
	mvstack.push(model_mat_cpp);
	model_mat_cpp = model_mat_cpp * translate(vec3(1.06, -0.04, -0.9)) * scale(vec3(0.05, 0.67, 0.05));
	setInt("color", 1);
	cube();
	model_mat_cpp = mvstack.pop();
...
void chairs(float tx, float ty, float tz, float sx, float sy, float sz, float tmp, int j) {
	mvstack.push(model_mat_cpp);
	//model_mat_cpp = model_mat_cpp * translate(vec3(tmp, 0.0, chair_table_z)) * translate(vec3(tx, ty, tz)) * scale(vec3(sx, sy, sz)) * rotate_y(360 / j);
	model_mat_cpp = model_mat_cpp * translate(vec3(chair_table_x, 0.0, chair_table_z)) * translate(vec3(tx, ty, tz)) * scale(vec3(sx, sy, sz)) * rotate_y(360 / j);
	chair();
	model_mat_cpp = mvstack.pop();
}
...
void DisplayFunc(void)
{
	...
	//ban
	mvstack.push(model_mat_cpp);
	model_mat_cpp = model_mat_cpp * translate(vec3(chair_tablex[table0], 0.1f, -5.5f)) * rotate_y(90);
	...
	chairs(1.5f, 0.0f, 0.2f, 0.9, 0.8, 0.4, chair_tablex[chair0], 1);
	model_mat_cpp = mvstack.pop();
...
void KeyboardFunc(unsigned char key, int x, int y)
{
	...
	case 'f':
		...
		if (chair_tablex[chair_table_x] > 0.75) { chair_tablex[chair_table_x] = 1.0; }*/
		if (chair_table_x <= 1.6) {
			chair_table_x += 0.1;
		}
		break;
	case 'r':
		...
		if (chair_table_x >= -2.0) {
			chair_table_x -= 0.1;
		}
		break;
	case 'u':
		if (chair_table_z <= 0.7) {
			chair_table_z += 0.1;
		}
		break;
	case 'p':
